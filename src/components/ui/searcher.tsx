            ...
            >
              <div className="col-span-2 w-full h-full space-y-4 overflow-auto overflow-x-hidden row-span-2 lg:row-span-1">
                {searching?.data?.data ? searching?.data?.data?.map((x: any, i: number) => (
                  <Link href={
                    selectedSearch === "store" ? `/stores/store/${x.id}`
                      : selectedSearch === "brand" ? `/brands/brand/${x.id}`
                        : selectedSearch === "productS" ? `/brands/brand/product/${x.id}`
                          : selectedSearch === "accounts" ? `/stores/store/${x.id}`
                            : ""} key={i}>
                    <div
