...
export default function SliderWithSkeleton() {
    const { data: sliderData, isLoading: isSliderLoading } = useGetHomeBannerQuery();

    const slides: SliderItem[] = sliderData?.data?.map((slider: RawSlider) => ({
        id: slider.id,
        image: slider.image,
        alt: `Slider Image ${slider.id}`,
    })) || [];

    return isSliderLoading ? <SliderSkeleton /> : <ProductCarousel slides={slides} />;
}