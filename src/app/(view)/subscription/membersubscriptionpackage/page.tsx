...
const Membersubscriptionpackage: NextPage = () => {
  ...
  const {
    ...
    error,
    refetch,
  } = useGetSubscriptionDetailsQuery({ type: "member" });
  const { data: memberAdons, isLoading: isLoadingAdons } =
    useGetmemberAdonsQuery();
    ...
    <div className="min-h-screen w-full bg-white text-left text-black font-sans">
      <div className="container mx-auto px-4 py-8 md:py-16">
        ...
        <main className="flex flex-col items-center gap-8">
          ...
          {paidPlan && (
            <section
              className={`w-full max-w-6xl rounded-lg bg-white p-6 md:p-8 transition-all border-2 ${
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-8">
                <div className="flex flex-col">
                  ...
                  <div className="mt-4 text-4xl md:text-6xl font-semibold text-black">
                    ${parseFloat(freePlan.price).toFixed(0)}/month
                  </div>
                  <div className="mt-auto pt-6 text-sm">
                    <p className="m-0">
                      <b>Badge: </b>
                      <span className="text-zinc-500">{freePlan.badge}</span>
                    </p>
                ...
                <div className="flex flex-col border-t lg:border-t-0 lg:border-l border-gray-200 pt-8 lg:pt-0 lg:pl-8">
                  ...
                  <div className="mt-auto pt-6 flex justify-end">
                    <button
                      className={`w-full sm:w-auto rounded px-6 py-2 font-medium text-sm cursor-pointer transition-colors ${
                        selectedPlanId === freePlan.id
          ...
          {paidPlan && (
            <section
              className={`w-full max-w-6xl rounded-lg bg-white p-6 md:p-8 relative transition-all border-2 ${
                isPaidPlanSelected
                  ? "border-purple-600"
                  : "border-black border-solid"
              }`}
                ...
                <div className="flex flex-col">
                  ...
                  <div
                    className={`mt-4 text-4xl md:text-6xl font-semibold ${
                      isPaidPlanSelected ? "text-purple-600" : "text-black"
                    }`}
                  >
                    ${parseFloat(paidPlan?.price).toFixed(0)}/month
                  </div>
                  <div className="mt-auto pt-6 text-sm">
                    ...
                    <p className="m-0 mt-2">
                      <b>Discounts/Perks: </b>
                      <span>
                        Real-world savings, voting power, and visibility
                      </span>
                    </p>
                  </div>
                </div>
          ...
          {hempAddon && (
            <section
              className={`w-full max-w-6xl rounded-lg bg-white p-6 md:p-8 transition-opacity ${
                isPaidPlanSelected
                  ? "opacity-100 border-black border"
                  : "opacity-50 bg-gray-50 border-gray-200 border"
              }`}
            >
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                ...
                <div className="flex flex-col items-start md:items-end flex-shrink-0 mt-4 md:mt-0">
                  <b className="text-3xl md:text-4xl">
                    ${parseFloat(hempAddon?.price).toFixed(0)}/month
                  </b>
