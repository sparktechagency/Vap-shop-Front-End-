...
const InvoiceModal: FC<{
  isOpen: boolean;
  onClose: () => void;
  onUpdateStatus: (id: number, newStatus: InvoiceStatus) => void;
  subscription: Subscription | null;
}> = ({ isOpen, onClose, onUpdateStatus, subscription }) => {
  const [currentStatus, setCurrentStatus] =
    useState<InvoiceStatus>("pending_invoice");
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg transform transition-transform duration-300 scale-100">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              <strong>Selected Plans:</strong> {subscription.plans}
            </p>
            <p className="text-lg font-bold text-black">
              <strong>Total Amount:</strong> ${subscription.total.toFixed(2)}
            </p>
          </div>
          <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-r-lg">
            <p className="text-sm text-yellow-700">
              <strong>Admin Action: </strong>Please log in to the Authorize.net
              dashboard and send an invoice of{" "}
              <strong>${subscription.total.toFixed(2)}</strong> to this user's
              email. Once the payment is received, update the status below
              accordingly.
            </p>
          </div>
          <div>
            ...
            <select
              ...
              value={currentStatus}
              onChange={(e) =>
                setCurrentStatus(e.target.value as InvoiceStatus)
              }
        ...
        <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
          <button
            ...
            className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-black text-base font-medium text-white cursor-pointer focus:outline-none sm:ml-3 sm:w-auto sm:text-sm"
          >
            Update & Save
          </button>
          <button
...
const Pagination: FC<{
  ...
  siblingCount?: number;
}> = ({ currentPage, lastPage, setCurrentPage, siblingCount = 1 }) => {
  const paginationRange = useMemo(() => {
    ...
    if (shouldShowLeftDots && !shouldShowRightDots) {
      ...
      let rightRange = Array.from(
        ...
        (_, i) => lastPage - rightItemCount + 1 + i
      );
      return [1, "...", ...rightRange];
    }
    if (shouldShowLeftDots && shouldShowRightDots) {
...
const SubscriptionsManagement: FC = () => {
  ...
  const {
    ...
    isError,
    refetch,
  } = useGetallSubscribedUsersQuery({ page: currentPage });
  ...
  const subscriptions: Subscription[] = useMemo(() => {
    return (
      apiResponse?.data?.data.map((sub: ApiSubscription) => ({
        id: sub?.id,
  ...
  const pendingCount = useMemo(
    () => subscriptions.filter((s) => s.status === "pending_invoice").length,
    [subscriptions]
  );
  ...
  if (isLoading)
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="text-xl font-semibold">Loading subscriptions...</div>
      </div>
    );
  if (isError || !apiResponse?.ok)
    return (
      <div className="flex justify-center items-center h-screen">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
            <div>
              ...
              <p className="text-gray-500 mt-1">
                Manage new subscription requests and billing.
              </p>
            </div>
                    ...
                    <tr key={sub.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <button
                          ...
                          className="bg-black cursor-pointer text-white px-4 py-2 rounded-lg shadow-sm hover:bg-gray-800 transition-colors"
                        >
                          Details / Manage
                        </button>
                      </td>
