...
const AdminSubscriptionEditor: NextPage = () => {
  ...
  const handlePlanChange = (
    planType: "store" | "brand" | "wholesaler",
    field: string,
    value: string
  ) => {
    setData((prev: any) => ({
      ...
      [planType]: {
        ...prev[planType],
        plan: { ...prev[planType].plan, [field]: value },
      },
    }));
  };
  ...
  const handleFeatureChange = (
    ...
    setData((prev: any) => ({
      ...
      [planType]: {
        ...
        plan: {
          ...prev[planType].plan,
          features: prev[planType].plan.features.map((f: Feature) =>
            f.id === featureId ? { ...f, text: value } : f
          ),
        },
      },
    }));
  ...
  const handleAddFeature = (planType: "store" | "brand" | "wholesaler") => {
    setData((prev: any) => {
      ...
      return {
        ...
        [planType]: {
          ...prev[planType],
          plan: { ...prev[planType].plan, features: updatedFeatures },
        },
      };
    });
  };
  ...
  const handleRemoveFeature = (
    ...
    setData((prev: any) => {
      const updatedFeatures = prev[planType].plan.features.filter(
  ...
  const handleMemberPlanChange = (
    ...
    setData((prev: any) => ({
      ...
      member: {
        ...prev.member,
        plans: prev.member.plans.map((p: Plan) =>
  ...
  const handleMemberFeatureChange = (
    planId: string | number,
    featureId: number,
    value: string
  ) => {
      ...
      member: {
        ...
        plans: prev.member.plans.map((p: Plan) =>
          p.id === planId
            ? {
                ...p,
                features: p.features.map((f: Feature) =>
                  f.id === featureId ? { ...f, text: value } : f
                ),
              }
            : p
  ...
  const handleMemberRemoveFeature = (
    planId: string | number,
    featureId: number
  ) => {
    setData((prev: any) => {
      const updatedPlans = prev.member.plans.map((p: Plan) => {
        if (p.id === planId) {
          return {
            ...p,
            features: p.features.filter((f: Feature) => f.id !== featureId),
          };
        }
        return p;
      });
  ...
  const handleGlobalAddOnChange = (
    ...
    setData((prev: any) => ({
      ...prev,
      globalAddOns: prev.globalAddOns.map((a: AddOn) =>
  ...
  const handleSave = async (item: Plan | AddOn, itemType: "plan" | "addOn") => {
    ...
    if (itemType === "plan") {
      ...
      else if (data.wholesaler?.plan.id === plan.id)
        currentItem = data.wholesaler.plan;
      else {
    ...
    try {
      const response = await updateAdonsAndPlan({
      ...
      <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
        ...
        <div className="space-y-4">
          <Input
            ...
            onChange={(e) =>
              handlePlanChange(planType, "title", e.target.value)
            }
          />
          <Input
            label="Price ($)"
            ...
            onChange={(e) =>
              handlePlanChange(planType, "price", e.target.value)
            }
          />
          <Textarea
            label="Description"
            ...
            onChange={(e) =>
              handlePlanChange(planType, "description", e.target.value)
            }
          />

          <h3 className="text-lg font-semibold text-gray-700 pt-4 border-t mt-6">
            Features
          </h3>
            ...
            {planData.plan.features.map((feature: Feature) => (
              <div key={feature.id} className="flex items-center gap-2">
                <input
                  ...
                  value={feature.text}
                  onChange={(e) =>
                    handleFeatureChange(planType, feature.id, e.target.value)
                  }
                  className="w-full text-sm p-2 border rounded-md"
                />
                <button
                  onClick={() => handleRemoveFeature(planType, feature.id)}
          ...
          <button
            onClick={() => handleAddFeature(planType)}
            className="w-full text-sm p-2 border-2 border-dashed rounded-md hover:bg-gray-50 text-gray-600"
          >
            + Add Feature
          </button>

          <button
            onClick={() => handleSave(planData.plan, "plan")}
            className="w-full mt-6 rounded bg-zinc-900 p-3 text-base font-medium text-white cursor-pointer hover:bg-zinc-700 transition-colors"
          >
    ...
    <div className="min-h-screen w-full bg-gray-50 p-4 sm:p-8">
      <header className="text-center mb-12">
        <h1 className="text-4xl font-extrabold text-gray-900">
          Admin Subscription Editor
        </h1>
        <p className="mt-2 text-lg text-gray-600">
          Edit all subscription plans and add-ons from one place.
        </p>
      </header>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200 lg:col-span-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {data.globalAddOns.map((addOn: AddOn) => (
              <div
                key={addOn.id}
                className="p-4 border rounded-lg space-y-3 bg-gray-50 flex flex-col"
              >
                <Input
                  label="Add-On Title"
                  value={addOn.title}
                  onChange={(e) =>
                    handleGlobalAddOnChange(addOn.id, "title", e.target.value)
                  }
                />
                <Input
                  label="Price ($)"
                ...
                />
                <button
                  onClick={() => handleSave(addOn, "addOn")}
                  className="w-full mt-auto rounded bg-black p-2 text-sm font-medium text-white cursor-pointer  transition-colors"
                >
                  Save Add-On
                </button>
              </div>
              ...
              {data.member.plans.map((plan: Plan) => (
                ...
                >
                  <Input
                    ...
                    value={plan.title}
                    onChange={(e) =>
                      handleMemberPlanChange(plan.id, "title", e.target.value)
                    }
                  />
                  <Input
                    label="Price ($)"
                    value={plan.price}
                    onChange={(e) =>
                      handleMemberPlanChange(plan.id, "price", e.target.value)
                    }
                  />
                  <Textarea
                    label="Description"
                  ...
                  />
                  <h4 className="text-md font-semibold text-gray-700 pt-2 border-t mt-4">
                    Features
                  </h4>
                  <div className="space-y-2">
                    {plan.features.map((feature: Feature) => (
                      <div key={feature.id} className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() =>
                            handleMemberRemoveFeature(plan.id, feature.id)
                          }
                          className="p-2 text-red-500 hover:bg-red-100 rounded-md"
                        >
                          &times;
                  ...
                  <button
                    onClick={() => handleMemberAddFeature(plan.id)}
                    className="w-full text-sm p-2 border-2 border-dashed rounded-md hover:bg-gray-50 text-gray-600"
                  >
                    + Add Feature
                  </button>
                  <button
                    onClick={() => handleSave(plan, "plan")}
                    className="w-full mt-auto rounded bg-zinc-900 p-2 text-sm font-medium text-white cursor-pointer hover:bg-zinc-700 transition-colors"
                  >
                    Save Plan
                  </button>
                </div>
