        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end gap-4">
            ...
            <button
              type="submit"
              disabled={isUpdating}
              className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-wait"
            >
            ...
            <tr key={category.id} className="hover:bg-gray-50">
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-sm text-center font-medium text-gray-900">
                  {category.store_products_count}
                </div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-sm text-center font-medium text-gray-900">
                  {category.wholesale_products_count}
                </div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-sm text-center font-medium text-gray-900">
                  {category.brand_products_count}
                </div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button
...
export default function CategoryManagement() {
  ...
  const [createACategory, { isLoading: isCreating }] =
    useCreateACategoryMutation();
  const [deleteACategory, { isLoading: isDeleting }] =
    useDelteACategoryMutation();
  ...
  const [categoryToEdit, setCategoryToEdit] = useState<ApiCategory | null>(
    null
  ); // New state for category being edited

  // 4. Memoized data processing
  const allCategories = useMemo(() => {
    return categoryResponse?.ok && categoryResponse.data
