...
export const adminApis = api.injectEndpoints({
  endpoints: (builder) => ({
    getallusers: builder.query<
      any,
      { page: number; per_page: number; role: number; searchterm: string }
    >({
    ...
    getallAddRequest: builder.query<
      any,
    ...
    approveAdd: builder.mutation<
      any,
      { id: string; type: string; status: string; is_active: string }
    >({
    ...
    adminResetPasswrod: builder.mutation<
      any,
    ...
    >({
      query: ({ id, params }) => {
        ...
        return {
          url: `/admin/update-ad-request-status/${id}?${queryString}`,
          method: "POST",
        };
      },
    }),
